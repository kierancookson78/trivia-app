...
class StatsFragment : Fragment() {
	...
	override fun onCreateView(
		...
		getNumberOfCorrectAnswersAndQuestions { correct, questions ->
			...
			percentageCorrectTextView.text = getString(R.string.correct_answer_percent, percentage)
		}
		return view
	}
	...
	private fun getBestTopic(callback: (String) -> Unit) {
		...
		var bestTopicString: String
		for (topic in topics) {
			// For each topic, get the percentage correct.
			database.collection("users").document(userId)
				.collection("topics").document(topic).get()
				.addOnSuccessListener { document ->
					val percentageCorrect = document.getLong("percentageCorrect")!!.toInt()
					if (percentageCorrect > bestPercentage) {
						// This topic has a higher percentage correct than the previous best topic.
						bestPercentage = percentageCorrect
						bestTopicString = topic
						callback(bestTopicString)
					}
				}
